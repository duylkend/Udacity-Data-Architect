...
SELECT TO_DATE(PERIOD, 'YYYYMMDD'),
   ...
   NORMAL_MIN,
   NORMAL_MAX
FROM "UDACITYPROJECT"."STAGING".TEMPERATURE_STG;
...
FROM "UDACITYPROJECT"."STAGING".PRECIPITATION_STG;
...
FROM "UDACITYPROJECT"."STAGING".review_stg;

INSERT INTO checkin(business_id, date)
SELECT parse_json($1):business_id,
   parse_json($1):date
FROM "UDACITYPROJECT"."STAGING".checkin_stg;
...
SELECT parse_json($1):business_id,
   ...
   parse_json($1):hours
FROM "UDACITYPROJECT"."STAGING".business_stg;
...
FROM "UDACITYPROJECT"."STAGING".covid_features_stg;
...
FROM "UDACITYPROJECT"."STAGING".TIP_STG;
...
FROM "UDACITYPROJECT"."STAGING".USER_STG;